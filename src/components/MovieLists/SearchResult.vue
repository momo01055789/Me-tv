<template>
  <div class="container" v-if="result">
    ...
    <div class="error" v-if="result.results.length < 1">
      <p>No search results avialable :_(</p>
    </div>
  </div>
</template>
  ...
  methods: {
    async FetchResults() {
      const response = await fetch(
        `https://api.themoviedb.org/3/search/movie?api_key=3990d54b09ed3c1179f3b38bd8567a99&language=en-US&page=1&include_adult=false&query=${this.query}`
      );
      this.result = await response.json();
    },
    async FetchSearch() {
      const response = await fetch(
        `https://api.themoviedb.org/3/search/movie?api_key=3990d54b09ed3c1179f3b38bd8567a99&language=en-US&page=1&include_adult=false&query=${this.search}`
      );
      this.result = await response.json();
      this.search = "";
    },
