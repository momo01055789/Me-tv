          ...
          <div class="production">
            ...
            <ul>
              <li v-for="(el, index) in data.production_companies" :key="index">
                <img
                  class="logo"
                  :src="url + el.logo_path"
                  alt="logoPoster"
                  v-if="el.logo_path !== null"
                />:{{ el.name }}
              </li>
            </ul>
          </div>
          <div class="cast" v-if="cast">
            ...
            <ul>
              <li v-for="(person, index) in cast.cast" :key="index">
                <img
                  class="person"
                  :src="url + person.profile_path"
                  v-if="person.profile_path !== null"
                  alt="castimg"
                />
                <h5>{{ person.name }} <b>As</b> {{ person.character }}</h5>
              </li>
            </ul>
          </div>
  ...
  methods: {
    async fetchdata() {
      const response = await fetch(
        `https://api.themoviedb.org/3/movie/${this.$route.params.id}?api_key=3990d54b09ed3c1179f3b38bd8567a99&with_cast`
      );
      this.data = await response.json();
    },
    async fetchCast() {
      const response = await fetch(
        `https://api.themoviedb.org/3/movie/${this.$route.params.id}/credits?api_key=3990d54b09ed3c1179f3b38bd8567a99`
      );
